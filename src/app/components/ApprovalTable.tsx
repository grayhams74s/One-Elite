...
import {
  ...
  import { MoreHorizontal, Eye, Pencil, Trash2, Check, Download, BanknoteIcon, Car, NotebookPen } from "lucide-react"
  ...
  export default function OrdersTable({ bookings, isLoading, onStatusUpdate }: OrdersTableProps) {
    ...
    const handleStatusUpdate = async (bookingId: number, status: string) => {
      ...
      return toast.promise(
        (async () => {
          const endpoint = session?.user?.role === 'staff' 
            ? `/api/approval/${bookingId}`
          ...
          // Only send email if user is admin or owner
        ...
        })(),
        {
          ...
          success: `Booking ${status}${session?.user?.role === 'admin' || session?.user?.role === 'owner' ? ' and notification sent!' : ' updated!'}`,
                        ...
                        <DropdownMenuContent align="end" className="bg-white text-black border-gray-100">
                          ...
                          {booking.status !== 'completed' && (
                            <>
                              ...
                              {session?.user?.role !== 'user' && booking.status !== 'approved' ? (
                                <DropdownMenuItem
                              ...
                              {booking.status !== 'approved' && (
                                <DropdownMenuItem
                                  ...
                                  onClick={() => handleStatusUpdate(booking.id, session?.user?.role === 'user' ? 'cancelled' : 'rejected')}
                                >
                                  <Trash2 className="mr-2 h-4 w-4" />
                                  {session?.user?.role === 'user' ? 'Cancel' : 'Reject'}
                                </DropdownMenuItem>
                              )}
                            </>
                          )}
